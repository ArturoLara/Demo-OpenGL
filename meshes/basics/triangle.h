...
public:
	...
	* @param position Vector de 2 componentes correspondiente al vertice superior del triangulo(x, y)
	...
	*/
	triangle(glm::vec2 position, glm::vec2 size, glm::vec4 color, GLuint shaderId, int layer = 0, int depth = 0, std::string type = "none");
	~triangle();
	/**
